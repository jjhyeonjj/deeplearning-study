...
print("------------------------------------------------------------------------")
# 각 정보별 특징 자세히 출력하기 (샘플 수, 평균, 표준편차, 최솟값, 백분위 수로 25%, 50%, 75%에 해당하는 값, 최댓값)
print(df.describe())
...
# 그래프 속성
'''
    시본 라이브러리 중 각 학목 간 상관관계를 나타내는 heatmap() 함수 통해 그래프 표시
        - 두 항목씩 짝을 지은 후 각각 어떤 패턴으로 변하는지 관찰하는 함수
        - vmax: 색상 밝기 조절하는 인자
...
# 모델 설정
model = Sequential()
model.add(Dense(12, input_dim=8, activation='relu', name='Danse_1'))
...
# 모델 실행(학습)
# 100번 반복한 결과 약 75%의 정확도를 보이고 있다
