'''
            ...
            - 경사 하강법(gradient descent)
                ...
                - mini batch
                    - 데이터를 미니배치 단위로 무작위로 추출해 경사 하강법을 수행
                        - 여러 데이터의 묶음
                        - 데이터를 하나씩 훈련하기보다는 여러 개를 한 묶음으로 처리(효율적)
                    - 미니배치 경사 하강법(mini batch gradient descent)

        6) 순전파 & 역전파
            - forward propagation
                - 신경망에서 입력값이 입력층과 은닉층을 거쳐 출력층에 도달하기까지의 계산 과정
                - 각 층을 통과할 때마다 입력값에 가중치를 곱해 다음 층으로 출력할 값을 게산
            - back propagation
                - 순전파의 반대 개념
                - 신경망 훈련에서 가장 중요한 내용
            ...
            - 절차
                ...
                - 기울기를 바탕으로 가중치를 갱신     이 때 출력층 => 입력층 방향으로 가중치를 갱신(역전파)
    ...
    2. 딥러닝 알고리즘 성능 향상
        1) 드롭아웃(dropout)
            - 과대 적합을 방기하기 위하여 신경망 훈련 과정에서 무작위로 일부 뉴런을 제외하는 기법
            - 얼마나 많은 뉴런을 드롭아웃할 지는 하이퍼파라미터로 설정할 수 있음
